...
split -l 1000 regions_enh_merged.brain.bed tmp.regions_enh_merged.brain.bed # split enhancer regions into 1000 length ??
for i in tmp.regions_enh_merged.brain.bed*; do echo $i; bsub -q short -n 1 -M 1000 bash ~/pipeline/bin/bed2narrowpeak.sh $i merged.DNase.pval.signal.bigwig; done # for each 1000 length, do bed2narrowpeak
