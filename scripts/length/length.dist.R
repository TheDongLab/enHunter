...
plus <- fread("./input_files/TNE/plus.length.bed", sep = "\t")

plus.med <- median(plus$V2, na.rm=TRUE)
...
plusLength
ggsave("./scripts/length/plusLength.png", plot=plusLength, device="png")

#system("awk '{OFS=\"\t\"; print $4,$3-$2}' minus.bed > minus.length.bed")
minus <- fread("./input_files/TNE/minus.length.bed", sep = "\t")

minus.med <- median(minus$V2, na.rm=TRUE)
...
minusLength <- ggplot(minus, aes(x=V2)) + 
  ...
  scale_y_continuous( n.breaks= 10) + 
  theme_bw() + 
  geom_vline(xintercept=minus.med, color="red", linetype="dashed") +
  theme(axis.line = element_line(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
minusLength
ggsave("./scripts/length/minusLength.png", plot=minusLength, device="png")
...
ggplot(minus, aes(x=V2)) + 
  ...
  geom_vline(xintercept=minus.med, color="red")
