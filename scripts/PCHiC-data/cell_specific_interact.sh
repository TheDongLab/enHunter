...
{ if ( NR == 1 ) {
  split($0, array); 
  col=0;
  for(i=12;i<=28;i++)if(a[i]==t){col=i};
...
}' PCHiC_peak_matrix_cutoff5.txt > $1.test.hg19.interact
...
cut -f 1,2,3,4 $1.test.hg19.interact | liftOver stdin hg19ToHg38.over.chain.gz $1.cor.hg38.bed unMappedPCHiC
awk 'BEGIN{OFS="\t"; FS="\t"} NR!=1{ print $9, $10, $11, $4, $13}' $1.test.hg19.interact | liftOver stdin hg19ToHg38.over.chain.gz $1.start.hg38.bed unMappedPCHiCstart
awk 'BEGIN{OFS="\t"} NR!=1{ print $14, $15, $16, $4, $18 }' $1.test.hg19.interact | liftOver stdin hg19ToHg38.over.chain.gz $1.end.hg38.bed unMappedPCHiCend
cut -f 5,6,7,8,4 $1.test.hg19.interact  > $1.color.tmp

### merge based on field 4 from above 
join -1 4 -2 1 <(LC_ALL=C sort -k 4,4b $1.cor.hg38.bed) <(LC_ALL=C sort -k 1,1b $1.color.tmp) | LC_ALL=C sort -k 1,1b | join -1 1 -2 4 - <(LC_ALL=C sort -k 4,4b $1.start.hg38.bed) | join -1 1 -2 4 - <(LC_ALL=C sort -k 4,4b $1.end.hg38.bed) | awk 'BEGIN{OFS="\t"} {print $2, $3, $4, $1, $5, $6, $7, $8, $9, $10, $11, $1, $12, $13, $14, $15, $1, $16}' > $1.hg38.intersect
...
bedToBigBed -as=$dir/interact.as -type=bed5+13 $1.hg38.intersect $dir/hg38.chrom.sizes $1.hg38.inter.bb
